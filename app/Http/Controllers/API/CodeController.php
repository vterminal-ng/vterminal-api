...
class CodeController extends Controller
{
    ...
    public function generateCode(Request $request)
    {
        ...
        dd(Code::where('customer_id', auth()->id())->where('status', CodeStatus::PENDING)->orWhere('status', CodeStatus::ACTIVE)->get()->count());
        // return failure if user has 1 active or pending code
        if (Code::where('customer_id', auth()->id())->where('status', CodeStatus::PENDING)->orWhere('status', CodeStatus::ACTIVE)->get()->count()) {
